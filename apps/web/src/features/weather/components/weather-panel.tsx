...
function readWeather(payload: unknown): TripWeather | null {
  if (isRecord(payload) && typeof payload.tripId === 'string' && Array.isArray(payload.days)) {
  ...
  if (
    ...
    typeof payload.data.tripId === 'string' &&
...
async function fetchWeatherState(tripId: string): Promise<WeatherState> {
  try {
    const response = await fetch(`/api/trips/${encodeURIComponent(tripId)}/weather`, {
      method: 'GET',
      headers: {
    ...
    const data = readWeather(payload);

    if (!data) {
      return {
        status: 'error',
        data: null,
        error: 'Meridian received an unexpected weather response.',
      };
    }
...
function conditionLabel(condition: WeatherCondition | null): string {
  switch (condition) {
    case 'CLEAR':
      return 'Clear';
...
function availabilityCopy(weather: TripWeather): {
  label: string;
  detail: string;
} {
  switch (weather.availability) {
    case 'AVAILABLE':
      return {
        ...
        detail: 'Weather coverage is available across your current journey window.',
      };

    case 'PARTIAL':
      return {
        label: 'Partial forecast',
        detail: 'Some trip days are still outside the current forecast window.',
      };

    case 'OUT_OF_RANGE':
      return {
        label: 'Forecast coming soon',
        detail:
    ...
    case 'LOCATION_NOT_FOUND':
      return {
        ...
        detail: 'Meridian could not resolve this destination to a forecast location yet.',
      };
  }
}
  ...
  if (!day.available) {
    return (
      <article className="rounded-[1.5rem] border border-white/[0.06] bg-white/[0.02] p-5">
        ...
        <p className="mt-4 text-sm font-medium text-slate-500">Forecast not available yet</p>
      </article>
    );
  }
    ...
    <article className="group rounded-[1.5rem] border border-white/[0.07] bg-white/[0.025] p-5 transition hover:border-white/[0.13] hover:bg-white/[0.04]">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-xs text-slate-500">{conditionLabel(day.condition)}</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-2 border-t border-white/[0.06] pt-4 text-xs text-slate-500">
        <div className="flex items-center gap-2">
          ...
          {formatMetric(day.precipitationProbabilityMax, '%')}
        </div>

        <div className="flex items-center gap-2">
          <Wind className="h-3.5 w-3.5 text-slate-400" />
          {formatMetric(day.windSpeedMaxKmh, ' km/h')}
        </div>
      </div>
    </article>
  );
...
function HeroForecast({ weather, day }: { weather: TripWeather; day: WeatherDay | null }) {
  const availability = availabilityCopy(weather);
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[linear-gradient(135deg,#0b2534_0%,#102b3a_50%,#09131e_100%)] p-7 sm:p-9">
        ...
        <div className="relative max-w-2xl">
          ...
          <h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-white sm:text-3xl">
            {weather.availability === 'LOCATION_NOT_FOUND'
              ? 'We could not locate this destination.'
              : 'Weather will appear closer to departure.'}
          </h2>

          <p className="mt-4 max-w-xl text-sm leading-7 text-sky-50/50">{availability.detail}</p>
          ...
          <div className="mt-7 inline-flex items-center gap-2 rounded-xl border border-white/[0.08] bg-black/10 px-4 py-3 text-xs text-slate-400">
            ...
            {formatCompactDate(weather.forecastWindow.requestedStartDate)}
            {' — '}
            {formatCompactDate(weather.forecastWindow.requestedEndDate)}
          </div>
        </div>
      </section>
    );
    ...
    <section className="relative overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[linear-gradient(135deg,#0c2938_0%,#123748_52%,#0a1721_100%)] p-7 sm:p-9">
      ...
      <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,0.55fr)] xl:items-end">
        <div>
          ...
          <div className="mt-10 flex items-center gap-5">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.055] text-sky-100">
              <ConditionIcon condition={day.condition} className="h-8 w-8" />
            </div>
          ...
          <div className="mt-8 flex items-end gap-3">
            ...
            <span className="pb-2 text-base text-sky-100/45">
              High · {formatTemperature(day.temperatureMinC)} low
            </span>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Metric
            icon={<Droplets className="h-4 w-4 text-sky-300/70" />}
            label="Rain chance"
            value={formatMetric(day.precipitationProbabilityMax, '%')}
          />

          <Metric
            icon={<Wind className="h-4 w-4 text-slate-400" />}
            label="Max wind"
            value={formatMetric(day.windSpeedMaxKmh, ' km/h')}
          />

          <Metric
            icon={<Sunrise className="h-4 w-4 text-amber-200/70" />}
            label="Sunrise"
            value={formatTime(day.sunrise)}
          />

          <Metric
            icon={<Sunset className="h-4 w-4 text-orange-200/70" />}
            label="Sunset"
...
function useWeather(tripId: string) {
  ...
  useEffect(() => {
    ...
    void fetchWeatherState(tripId).then((nextState) => {
      if (!cancelled) {
        setState(nextState);
      }
    });
...
export function WeatherPanel({ tripId }: { tripId: string }) {
  const { state, refresh } = useWeather(tripId);

  const availableDays = useMemo(
    () => (state.status === 'success' ? state.data.days.filter((day) => day.available) : []),
    [state],
  );
  ...
  const heroDay = availableDays[0] ?? null;
  ...
  return (
    <div className="py-9">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
            Meridian uses your destination and trip timezone to line up daily forecast context with
            the days you are actually traveling.
          </p>
        </div>
      ...
      <div className="mt-7">
        <HeroForecast weather={weather} day={heroDay} />
      </div>
        ...
        <>
          <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-white">Your travel window</p>
            </div>

            <p className="text-xs text-slate-600">Provider · Open-Meteo</p>
          </div>
      ...
      {heroDay && (
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <Metric
            icon={<ThermometerSun className="h-4 w-4 text-sky-300/70" />}
            label="Temperature"
            value={`${formatTemperature(heroDay.temperatureMaxC)} / ${formatTemperature(
          ...
          <Metric
            icon={<Droplets className="h-4 w-4 text-sky-300/70" />}
            label="Precipitation"
            value={formatMetric(heroDay.precipitationMm, ' mm')}
          />
...
export function WeatherJourneyStrip({ tripId }: { tripId: string }) {
  const { state, refresh } = useWeather(tripId);
    ...
    return (
      <div className="flex flex-col gap-4 rounded-[1.5rem] border border-white/[0.07] bg-white/[0.025] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">Weather context is unavailable right now.</p>
        </div>
  ...
  const previewDays = weather.days.slice(0, 5);

  const firstAvailable = previewDays.find((day) => day.available);
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              {weather.availability === 'LOCATION_NOT_FOUND'
                ? 'Check the journey destination and try again. Meridian needs a resolvable city or place for weather.'
    ...
    <section className="overflow-hidden rounded-[1.5rem] border border-white/[0.07] bg-white/[0.025]">
      <div className="flex flex-col gap-3 border-b border-white/[0.06] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-white">Conditions beside your itinerary</p>
        </div>

        <div className="inline-flex items-center gap-2 text-xs text-slate-500">
          <MapPin className="h-3.5 w-3.5" />

          {weather.location ? weather.location.name : weather.destination}
        </div>
      </div>

      <div className="grid divide-y divide-white/[0.06] sm:grid-cols-5 sm:divide-x sm:divide-y-0">
        {previewDays.map((day) => (
          <div key={day.date} className="flex items-center justify-between gap-4 p-4 sm:block">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                {day.available ? conditionLabel(day.condition) : 'Pending'}
              </p>
            </div>

            <div className="flex items-center gap-3 sm:mt-4">
              <div className={day.available ? 'text-sky-200' : 'text-slate-700'}>
                <ConditionIcon condition={day.condition} className="h-5 w-5" />
              </div>

              <p className="text-sm font-semibold text-white">
                {day.available ? formatTemperature(day.temperatureMaxC) : '—'}
              </p>
            </div>
          </div>
        ))}
