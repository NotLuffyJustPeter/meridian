...
if (
  !Number.isInteger(PRE_ALLOCATED_VUS) ||
  PRE_ALLOCATED_VUS <= 0
) {
  throw new Error(
    'PRE_ALLOCATED_VUS must be a positive integer.',
  );
}

if (
  !Number.isInteger(MAX_VUS) ||
  MAX_VUS < PRE_ALLOCATED_VUS
) {
  throw new Error(
    'MAX_VUS must be an integer greater than or equal to PRE_ALLOCATED_VUS.',
  );
}
